...
class SupabaseService {
  ...
  Future<void> checkAndUpsertPlayer({
    ...
    try {
      ...
      if (response.isNotEmpty) {
        ...
        // Solo actualizamos si el nuevo score es mayor (Highscore logic)
        // O si el usuario quiere sobreescribirlo siempre?
        // El prompt dice "guardar tu score", asumiremos que es tipo Highscore personal
        // o simplemente guardar el último. El prompt dice "guardar tu score",
        // pero para "Highscores" normalmente quieres el máximo.
        // Voy a asumir que actualizamos si es mayor para mantener el "Highscore".
        // Sin embargo, el código del usuario original NO tenía esa lógica, solo update.
        // Voy a mantener la logica del usuario: SIEMPRE actualiza.
        // (Aunque para un ranking Top 3, esto podría significar que si juegas mal pierdes tu record).
        // EDIT: Para proteger el highscore, mejor comprobamos.
        ...
        if (score > existingPoints) {
          ...
          debugPrint(
            'Jugador $playerName tiene $existingPoints. El nuevo score $score no es mayor. No se actualiza.',
          );
        }
      } else {
  ...
  /// Obtiene los mejores 3 puntajes.
  Future<List<Map<String, dynamic>>> getTopHighscores({int limit = 3}) async {
    try {
      final response = await _client
          .from('players')
      ...
      debugPrint('❌ Error fetching highscores: $e');
      return [];
    }
  }
}
